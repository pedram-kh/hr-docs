...
- **A hand-built gold set only tests what the builder saw** (10c, jornada checkpoint). A passing eval bounds *known* failure modes, not unknown ones — it proves nothing about content the builder never sampled. When a model reports an *absence* ("no dedicated article for X"), that claim is exactly as trustworthy as the input it was given: verify the input actually contained the content **before** believing the model's negative, and before "fixing" it with a prompt rule. 10c's convenio-25 false negative looked like a reasoning failure and got a prompt-rule fix written for it; only checking the actual passage-scoped input size against the cap (not the model's behavior) found the real bug — the article had never been sent at all. The prompt rule shipped anyway as defense-in-depth, but it was not the fix.
- **Parallel code paths with divergent instrumentation are a recurring trap** (10c). `propose_groups()`'s input-truncation cap was logged (`text_truncated`); the newer topic-scoped segmentation path added alongside it (same sprint family, same shape — a `[:CAP]` slice on model input) copied the truncation *logic* but not the trace *signal*, so the exact same failure mode existed with zero visibility for months. It recurred in miniature even earlier: `/ground`'s truncation-retry (`grounding_truncated`) predated `/synthesise`'s own (`synthesis_truncated`), which only reached parity in Sprint 10b (ADR-0033) once the gap was noticed. **When adding a code path alongside an existing one that does something similar, port its instrumentation, not just its logic** — a new path is exactly where an old lesson gets silently un-learned.
...
- **Go-live data pass** (HR, via the review queues — now ranked by Analítica): assign employee groups (directory picker / CSV `group` column `Grupo 2 > resto áreas`); verify the 82 facts; bind facts 41/79; approve the 5 pending group trees; confirm scope on `UNDER_REVIEW_SCOPE` docs (COEAS Estatal, Madrid, Huesca); **topic-tag the active convenios** (no doc has a topic tag → the structural fence blocks every ruling publish, the flywheel is dormant); source missing base texts (convenio 9's IV convenio; Cultura Navarra needs a real BOE code in the registry); re-supply salary xlsx where only PDFs exist.
  - **Named example of the judgment calls HR will actually face** (Sprint 10c, jornada `needs_review` queue, convenio 18/ACCIÓN E INTERVENCIÓN SOCIAL): two AI-proposed facts sit side by side with near-identical but not-identical group labels — "Personal en régimen de guardia o expectativa" and "Personal en régimen de guardia o expectativa (disponibilidad)" — each with its own figures. Because the labels differ as strings, they didn't collide at the DB layer (no silent overwrite; both survive as separate `needs_review` rows), but nothing in the system can tell HR whether this is the *same* group described twice from two different passages (merge, verify one, reject the other) or two genuinely distinct sub-populations (verify both). This is exactly the class of call the AI is deliberately never allowed to make (ADR-0020) — flagging it here as a concrete, already-live instance for whoever runs the data pass to resolve, not a bug for engineering to fix.
  - **Coverage property, not a gap** (Sprint 10c, topic 4/festivos, CP-4): measured directly, not assumed — only 3 of 15 eligible convenios (20%) have any genuine festivos day-count/calendar content at all; the other 12 correctly, verifiably yield zero facts because they only ever mention "festivos" in retribución clauses (a pay premium for working one), never a día-de-cierre grant. **Festivos employee questions will therefore keep escalating even once this topic's 3 facts are fully verified** — there is no missing fact to extract for most convenios, because most convenios genuinely don't grant one. Full corpus-shape writeup in `sprints/sprint-10c/review.md`'s festivos sections and `sprints/sprint-10c/data-pass-handoff.md`.
