...
### §5 addendum — Round 2: decisions on both findings, doc 57, doc 59/94

Pedram's decision on finding 1 (bilingual duplicate): keep only the `es` table, drop the `eu` duplicate rows, note it. Decision on finding 2: do **not** touch `salary.py`'s header matching (units are semantically meaningful, mes vs año) — instead PROPOSE a mapping and stop for approval. Both implemented in `salary:pdf-to-xlsx` itself (no `salary:import`/`salary.py` change):

1. **Bilingual dedup, automatic.** A page's `table_rows` is now scanned for a SECOND row whose first cell normalizes to exactly `"categoria"` (the same accent/case-insensitive normalization as `salary.py`'s own `_norm()`, mirrored in PHP). When found (index > 0), every row before it is dropped from the grid and logged **verbatim** to the `Notes` sheet as `dropped_bilingual_duplicate`; a no-op otherwise. Verified live on both docs: doc 12 and doc 27 each dropped exactly 6 `eu` rows (header + 5 categories), keeping only the 6 `es` rows — confirmed by reading the Notes sheet back from S3.
2. **Header-mapping proposal, printed, never applied automatically.** A `CANONICAL_VOCAB` constant mirrors `salary.py`'s `_GROSS`/`_HOURLY`/`_EXTRA`/`_NIGHT`/`_RAW_MONEY` exact-match sets — read-only, kept only to propose a mapping to a human. A 4-tier cascade (exact match → strip currency/unit parens → strip all parens → substring/token match) proposes a canonical replacement per header cell, printed in the command's default (still read-only) output next to each OCR'd header cell. A safety rule refuses (`unsafe`, not proposed) any mapping that would drop a MONTHLY qualifier while landing on the strictly-ANNUAL `gross_annual` field — the exact mes/año conflation risk. Applying an approved proposal is a *separate* invocation, `--apply-header-mapping`, which mutates ONLY the header row of the already-written derived `.xlsx` (data rows untouched, idempotent). A new `--verify` flag calls the same `ExtractionClient::extractSalary()` `salary:import` itself calls, read-only, to confirm detection before committing to a real import.

**Proposed mapping — docs 12 and 27 (identical header row, both convenio 15):**

| OCR'd header cell (verbatim) | proposed | target field | dropped |
|---|---|---|---|
| `Categoría` | *(unchanged — label column)* | — | — |
| `Salario base (mes)\n(€)` | `Salario base` | untyped anchor (never `gross_annual`/`base_salary_monthly` — those are computed from the *other* column) | `(mes)` [semantic], `(€)` [currency] |
| `Pagas extras (€)` | `Pagas extras` | `extra_pay` (typed) | `(€)` [currency] |
| `Salario anual (€)` | `Salario anual` | `gross_annual` (typed — drives the `/14` monthly computation) | `(€)` [currency] — `anual` is *kept*, consistent with the field's own annual semantics |
| `Valor hora (sin antigüedad)\n(€/hora)` | `Hora` | `hourly_rate` (typed) | `(sin antigüedad)` [semantic — a real, non-currency information loss: the stored figure loses the "excludes seniority" caveat, though the number itself is still correctly hourly, not conflated with any other period] , `(€/hora)` [currency] |

**Awaiting approval — not yet applied, not yet imported for either doc.**

**Doc 57 (convenio 3, Ocio Educativo Álava, the current 2026 table) — OCR'd for the first time; structurally different, no mapping proposed.** 3 pages, `es`-only (no bilingual duplicate — Álava's gazette is Spanish-only here, unlike Gipuzkoa's). Page 2 (24 rows) + page 3 (8 rows, same table continued) both header `CATEGORÍA | 2025 | 2026` — **columns are validity YEARS, not field types**, and every money cell carries a literal `" euros"` suffix (e.g. `"36.343,74 euros"`), not a bare number. This is not the same shape as docs 12/27's problem: relabeling the header cannot fix it, because `salary.py` has no header-vocabulary concept of "a year" as a field at all — the table would need to be split into two single-column sheets (one per year, each a `Salario anual` column) and the `" euros"` suffix stripped from every cell, which is a structural transform, not a per-cell header rename, and is outside what `--apply-header-mapping` does (by design — it only ever touches row 1). The command's own proposal output correctly reports "no canonical vocabulary term found" for all three header cells rather than forcing a bad match. **Flagged for a human decision on how (or whether) to proceed; nothing applied, nothing imported.** Derived document `[104]` (uuid `9899d80a-9e86-4a9c-b7e9-d1077c3ab200`) exists with the verbatim, unmodified grid for reference.

**Doc 59 skipped** (convenio 3, the 2023–25 predecessor table — stale per the §5 note above, superseded by doc 57's 2026 table; not converted). **Doc 94 skipped** (convenio 5, Deporte Asturias) — logged again here: convenio 5's own prose has been `EXPIRED_NO_SUCCESSOR` since 2012 (corpus-coverage.md Part 4), so there is no current prose scope for doc 94's salary numbers to attach to; converting it now would produce figures for a scope with no live successor. Held pending that separate registry/scope resolution.

Tests: `Sprint7eSalaryPdfToXlsxTest.php` grew from 3 to 6 (bilingual-dedup-and-Notes-logging, propose-then-apply-on-a-separate-step-with-idempotent-reapply, monthly-onto-annual-unsafe-guard) — full suite 122/122 green.

### §5 addendum — Round 3: the approval condition, and docs 12/27 imported

Pedram approved the mapping above **with one condition**: the original header text must be preserved verbatim in `raw_values` for every mapped column, and the dropped qualifiers recorded on the Notes sheet. Both implemented; docs 12/27 then went through the full sequence and convenio 15 is salary-answerable.

1. **The condition, and where it had to live.** `salary.py` keys `raw_values` by the **normalized** header cell it reads (`raw_values[label]`, `label = _norm(header[c])`), so once `--apply-header-mapping` rewrites row 1 to `Hora`, the imported row knows only `"hora"` — the source's `"Valor hora (sin antigüedad)\n(€/hora)"`, and with it the "excludes seniority" caveat, existed nowhere in the database. It cannot be fixed in the sheet (an exact match is exactly what the header rewrite is for) and `salary:import`/`salary.py` are off-limits, so it is done in the step that already runs after the import for provenance: `--apply-header-mapping` now writes a machine-readable `header_mapping_manifest` row to the Notes sheet (alongside a human-readable `header_mapping_applied` line per column listing the exact qualifiers dropped, each tagged `[currency]` or `[semantic]`), and `--mark-provenance` reads it back to add each column's original text as an **additional** `raw_values` key holding the same verbatim value. The importer's own key is never removed or rewritten; the operation is idempotent. Because `salary:import` deletes and rewrites rows, **`--mark-provenance` must be re-run after any re-import** — the command says so in its own output.
2. **A silent data-loss bug found while reading `salary:import` for this change, and fixed.** Derived sheets were named `"Page N"`. `salary.py`'s `_year_from_sheet_name()` finds no year in that string, so `salary:import` would have written `year = NULL` — and it keys `SalaryTable::updateOrCreate(['convenio_id', 'year'])` and then **deletes and replaces that table's rows**. Importing doc 27 (2026) would therefore have silently deleted doc 12's (2025) rows for convenio 15: two source documents collapsing onto one table row, with no error and no warning. Derived sheets are now named `"<year> (page N)"`, the year read from the page's own OCR'd title first (`"2025EKO SOLDATA-TAULAK"` / `"TABLAS SALARIALES 2025"`), then the document title, filename, and `validity_start` — never inferred from the grid's figures — with its provenance recorded on the Notes sheet. Two sheets resolving to the same year now warn explicitly. This is the one case in this feature where the derived `.xlsx` contains a string that is not verbatim OCR output; it is a sheet *name*, never a cell, and it is the only channel through which `salary:import` can learn a year.
3. **The run (staging, docs 12 and 27).** Rebuild → `--apply-header-mapping` (4 header cells each) → `--verify`: doc 12 `sheet '2025 (page 2)': year=2025, 5 rows`, doc 27 `sheet '2026 (page 2)': year=2026, 5 rows` — the grid that was invisible to the parser two rounds ago is now detected, with no change to `salary.py`. Then the unmodified `salary:import`: **2 `salary_tables`, 10 rows, 5 new job categories** (Grupo I–V), followed by `--mark-provenance`: both tables `source = ocr_pdf`, and 20 verbatim header keys restored per document (4 columns × 5 rows).
4. **Convenio 15, after.** `salary_tables` `[14]` year 2025 and `[15]` year 2026, both `source = ocr_pdf`, `source_document_id` → the derived docs 102/103, which carry `derived_from_document_id` → the original PDFs 12/27. 5 rows each; 5 categories. Verified against the page images: Grupo I 2025 = `2.160,38 / 2.160,38 / 32.405,77 / 20,35`, matching the gazette cell for cell. Each row's `raw_values` carries both keys, e.g. `"hora" => "20,35"` and `"Valor hora (sin antigüedad)\n(€/hora)" => "20,35"`.
5. **End-to-end through the chat.** A convenio-15 test employee (`job_category_id = 97`, Grupo I) asking *"¿Cuál es mi salario base mensual y mi salario anual según las tablas salariales vigentes?"` via the real `POST /chat/message`: `floor_decision.path = "salary_sql"`, `outcome = answer`, `year_selection = "exact"` on year 2026, citation → document 103 with `is_salary_table = true`. The salary SQL path now serves a convenio whose figures only ever existed in a PDF.
6. **One accuracy caveat this exposed — not an OCR artifact.** The answer states `salario base mensual de 2.392,24 €`, but the gazette prints `2.232,75 €` for that category. `salary.py` derives `base_salary_monthly = gross_annual / 14` unconditionally, and this convenio's annual is **15 ×** its printed monthly base (2.232,75 × 15 = 33.491,25 ≈ 33.491,36), so the derived monthly is ~7 % high. This affects native `.xlsx` imports identically — it is a pre-existing property of the `/14` canonical rule, not of this path — and it is precisely what the verbatim-`raw_values` condition protects against: the gazette's own figure is in the row, so it is fixable without re-OCR. Recorded as go-live item (d) in `deploy.md` §5. **Resolved in §7 below (Correction-salary-01, ADR-0027) — the caveat turned out to affect 21 rows across three convenios, not just this one.**

**Doc 57 — the caption text, as requested; nothing applied.** Page 2's only title is `"Subida salarial año 2026"`. The prose around the grid is the *acta* of the comisión paritaria: it invokes anexo I of the *Convenio Colectivo de Ocio Educativo y Animación Sociocultural de Álava (años 2023-2026)* (BOTHA 142, 16 Dec 2024), agrees "la actualización de las tablas salariales correspondientes al año 2026 … que tendrán efectos desde el 1 de enero de 2026", and closes with the uprate arithmetic ("… el incremento salarial será de un 3,9 por ciento"). Page 3 carries no title, only the footnote block: `Plus de domingos y festivos: 4,50 euros/hora`, `Festivos de especial significación: 5,14 euros/hora`, `Compensación de gastos: 0,38 euros/km`, `Antigüedad: por trienio 26,01 euros en 14 pagas`. **Nothing on either page says "anual" or "mensual" about the two year columns.** What the text does establish: the columns are the same quantity one year apart (36.343,74 × 1,039 = 37.761,14 exactly, and 26.295,76 × 1,039 = 27.321,29), and the convenio's own antigüedad plus is expressed "en 14 pagas". Interpreting the magnitude is a human reading; the tool proposes nothing. Two further reasons doc 57 must not be imported as-is, confirmed by reading `salary.py`: `_looks_like_hours_header()` matches any bare 3–4 digit header, so `2025`/`2026` would be typed as the **hourly-rate** column, and `_is_number("36.343,74 euros")` is false, so every data row is discarded — the import would report success and write **zero** rows.

Tests: `Sprint7eSalaryPdfToXlsxTest.php` 6 → **8** (verbatim-`raw_values`-restored-and-idempotent across all four mapped columns incl. the Notes-sheet qualifier record and the year-from-page-title sheet name; same-year-collision warning) — full suite **124/124** green.
...
## §7 — Correction-salary-01: a salary figure is a source cell, or it is not stored (ADR-0027)

§5's closing caveat — convenio 15's chat answer stating `2.392,24 €` where its gazette prints `2.232,75 €` — was not a rounding artifact of the OCR path. It was `salary.py` computing `base_salary_monthly = gross_annual / 14` for every row since Sprint 2a, and the OCR path was simply the first import where a human had the printed monthly in front of them to compare against.

### 7.1 The audit, before anything was changed

Every `salary_table_rows` row was compared against what its own `raw_values` states. 32 rows print a monthly next to an annual, i.e. are checkable at all; **the `/14` rule contradicted the source on 21 of them**:
...
*(Figures are the first category of each table; "error" spans the best and worst row in it. "Implied pagas" is annual ÷ printed monthly — none of these convenios pays in 14.)*

Two findings worth keeping:

- **The 11 rows the rule got right are why it survived three sprints of review.** Convenio 6 (Deporte Cantabria) prints `Salario base` = 2.575,00 and an annual of 36.050,00 — exactly 14 ×. Anyone spot-checking the derivation against that sheet would have confirmed it. The rule was not unreviewed; it was reviewed against the one corpus member that satisfies it.
- **Convenio 10 shows why "a monthly" is not one quantity.** Its sheet prints both `salario base` 1.183,34 and `bruto mes` 1.771,64 (base + prorated extra + transport + a 5 % improvement). `base_salary_monthly` takes the base; the `/14` figure of 1.518,55 sat between the two, matching neither, which is exactly what a computed figure looks like when it has no referent.

### 7.2 What changed, and the standing guard

`salary.py` no longer derives (ADR-0027 for the full decision): monthly comes from a labelled monthly column or stays NULL, `pagas_count` (renamed from `num_payments`, which invited the old behaviour back by its name) is stored only where a header states it and never divides anything. `SalaryAnswerService` states the annual and the pagas count and omits the monthly when there is none, rather than filling the gap. `salary:audit-monthly` is now a read-only command that re-runs §7.1's comparison and exits non-zero on any discrepancy, any monthly with no source cell behind it, and any `pagas_count` no header states.

All 8 salary documents were re-imported and the three OCR-derived ones re-stamped with `--mark-provenance`. Post-fix audit: **0 failures across 179 rows in 14 tables, 107 of which carry a monthly the source itself states.** One coverage note remains, deliberately: COEAS Navarra's 2024 table (32 rows) has a source sheet that prints `14 pagas` and `12 pagas` *twice*, once per year, in one flat grid — so no monthly can be attributed to the table's single year, and none is typed. Fixing that means splitting the source workbook per year, which is a decision about the source, not the code.

Convenio 15 end-to-end after the correction, same employee and question as §5.5: `floor_decision.path = "salary_sql"`, citation → document 103 (`is_salary_table = true`), and the answer now reads *"bruto anual de 33.491,36 €; salario base mensual de **2.232,75 €**"* — the gazette's own cell. **Before/after: 2.392,24 € → 2.232,75 €.**

### 7.3 The golden-trace re-baseline (deliberate, not drift)

`Sprint7cAdditivityRegressionTest`'s originally pinned salary turn is **byte-for-byte unchanged**: its fixture stores both figures, and the correction only ever removes figures that were never sourced. The behaviour that did change is pinned as a **new** turn in the same test — a source with an annual and no monthly now answers with the annual alone, where it previously appended a derived *"salario base mensual de 1.500,00 € en 14 pagas"*. The live before/after is convenio 15's 2.392,24 € → 2.232,75 €. Recorded here and in ADR-0027 as a correctness re-baseline: the previous baseline pinned a figure that appeared in no source document.

### 7.4 `salary:import` now fails loudly (priority 2)

hr-ai returns a per-sheet `sheet_diagnostics` block (`ok` | `empty` | `no_header` | `header_but_no_rows` | `header_maps_to_nothing`) and `salary:import` refuses the document — non-zero exit, no success line, nothing written — on either of the last two. A notes or junk sheet with no header stays benign, as designed. This immediately caught a real case on staging: document 104 (doc 57's derived file) had been sitting "imported" with zero rows since §5, which is precisely how a coverage gap disguises itself as a completed one.

### 7.5 Doc 57 converted (priority 3)

Its grid is `CATEGORÍA | 2025 | 2026` — one column per year, every cell reading `36.343,74 euros` — which §5 correctly refused to guess at. The approved shape is now a flag, `salary:pdf-to-xlsx --year-columns`: one sheet per year, the year column's values placed under the canonical `Salario anual` header, the grid's pages merged (the categories span pages 2–3), and the literal `" euros"` suffix removed so the cell is a number to the importer. Digits, separators and order are untouched, both alterations are recorded on the Notes sheet, and the untouched grid stays in the OCR sidecar. No monthly is written: the source states none.

Result: **+2 `salary_tables` (`source = ocr_pdf`), +60 rows, +30 job categories**, and convenio 3 moves from prose-only to salary-answerable. Its answer is annual-only by construction — *"Para la categoría Director/a gerente … bruto anual de 37.761,14 €"* — which is the §7.2 behaviour doing its job on real data rather than a fixture. (Verified against the page: the acta's own uprate arithmetic, 36.343,74 × 1,039 = 37.761,14, holds exactly.)

### 7.6 Tests

`hr-ai/scripts/salary_parser_test.py` is new (9 scenarios pinning the parser contract directly: annual-only yields NULL monthly; a monthly column sources it; `pagas_count` read from a header; several stated monthlies preferring 14; contradictory counts yielding NULL; a repeated header suffixed in `raw_values` with no monthly typed; `sheet_diagnostics` separating a benign notes sheet from a recognized-but-empty grid). `CorrectionSalary01Test.php` is new (13 tests: the answer service's shapes with and without a stored monthly and a stated pagas count, `salary:import`'s three loud failures and its benign notes-sheet case, and the audit's discrepancy / unsourced / clean / coverage-not-failure outcomes). `Sprint7eSalaryPdfToXlsxTest.php` 8 → 9 (`--year-columns`), `Sprint7cAdditivityRegressionTest.php` 2 → 3 (§7.3). Backend suite **139/139 green** (702 assertions), plus the hr-ai parser script passing standalone.
