...
- **Two real bugs found in this generated export by cross-checking it against that same archive, fixed same-day (§13 item 12 has the full writeup): a `SCAN_NO_TEXT`/`UNDER_REVIEW_SCOPE` misclassification in `proseCell()` (zero chunks ≠ no text — `chunks:embed` also skips `under_review` tagging regardless of text quality), and the `DEV-FIXTURE-0001` seed convenio polluting the grid/headcount with fake data. Both fixed, tested (3 new regression fixtures), redeployed, and re-exported.**
...
- *Calidad*: originally built as the review tab (Step 6); **promoted to its own top-level `AdminShell` view during the eyes-on follow-up** (2026-09-10, §14) after the eyes-on found it unreachable from the top-level nav.
- Nav gated on `canViewAnalytics`/`canViewCoverage` (Step 9's ability), mirroring `AdminShell.tsx`'s existing gating pattern exactly (nav hidden **and** body re-checks the ability server-side via the endpoints, not just client-side). **§14 corrects a real bug here**: the ability payload itself (`IdentityPresenter`) never carried `analytics.view`, so this gating was broken for every role until the eyes-on caught it.
...
1. **Committed and pushed `sprint-8`** to all four remotes (one commit per repo for the build session's work, plus one follow-up commit each on `hr-backend`/`hr-docs` for this staging session's own changes — the τ config refactor and the doc updates you're reading now). Final SHAs, deployed: `hr-backend` `392fa4d`, `hr-ai` `d1b7d3f`, `hr-frontend` `6e561af`, `hr-docs` `a70fdf1`.
2. **Snapshot `hr-staging-pre-8`** — RDS manual snapshot, taken and confirmed `available` before any deploy.
3. **`deploy.sh` with the four `sprint-8` SHAs** — ran clean: leak scan passed, images built, **all 9 migrations ran** (`coverage_snapshots`, `analytics_daily_rollups`, `question_clusters`/`question_cluster_members`, `quality_samples`, the `quality_sample_wrong` reason addition, the `analytics.view` permission seed, `message_feedback` — plus the pre-existing 7g `escalation_cards` explanation-columns migration that a fresh clone always re-applies as part of the same batch), `hr-backend-scheduler` container created, full stack came up, health-check loop went green on attempt 2/90 (~10s). **Re-deployed a second time** (same procedure, same 4-SHA form) after this session's own `hr-backend`/`hr-docs` follow-up commits (τ config, doc updates) — health green again on attempt 3/90, `config('hr.question_cluster_threshold')` confirmed `0.8` live on the running container.
...
12. **Two real bugs in `CorpusCoverageService`, found by cross-checking item 10's export against the archived hand-run ledger, fixed and re-verified live — same day, before the eyes-on:**
    - **Reason-code bug.** `proseCell()`'s zero-chunk branch unconditionally returned `SCAN_NO_TEXT` for any active prose doc with 0 chunks, without checking *why* it had 0 chunks. Confirmed live: convenios 11 (`99100055012011`, COEAS Estatal) and 17 (`28102145012018`, COEAS Madrid) both have full page-level text (74/74 and 49/49 `pages_with_text`) but 0 chunks, because `chunks:embed` skips `tagging_status = under_review` docs regardless of text quality — a tagging block, not a scan/OCR problem. Fixed by checking `document_pages` text presence before falling back to `SCAN_NO_TEXT`; both rows now correctly read `UNDER_REVIEW_SCOPE`. Two new regression fixtures added to `CorpusCoverageAgreementTest` (one for each cause, so the fix can't just move the bug the other direction).
    - **Fixture-pollution bug.** The `DEV-FIXTURE-0001` seed convenio (`TestUserSeeder`'s Sprint-0 test-employee FK scaffolding) was appearing in the grid as a 27th row with a real headcount (1). `grid()`/`headcounts()` now exclude any `DEV-FIXTURE-%` convenio (new `DEV_FIXTURE_NUMERO_PREFIX` constant); a third regression fixture covers it. Added to `deploy.md` §4's scrub-before-production checklist, since the underlying row/employee shouldn't exist in a production DB at all — the code fix is a safety net, not the real remediation.
    - **Re-verified for real**: full local suite green (576 tests, up from 575), `corpus:coverage --print` re-run live on staging after redeploying the fix — output confirmed both reason codes corrected and the fixture row gone (26 rows, was 27) — and `corpus-coverage.md` regenerated again from that real output. **Third redeploy this session** (`hr-backend` `ea674eb`, `hr-ai`/`hr-frontend` unchanged, `hr-docs` `fcd114b`'s parent `6b3922d` at deploy time) — health green on attempt 2/90.
...
## 14. Eyes-on round 1 — two real access/render bugs, found by actually logging in (2026-09-10)

Before finishing the browser walkthrough, logging in as `admin@hr-staging.internal` (`super_admin`) surfaced two real bugs neither the API-matrix tests nor a route-hit test could have caught, because both are about what the FRONTEND does with a correct server response, not whether the server enforces access correctly:

1. **Analítica/Calidad missing from the nav for super_admin.** Cobertura rendered; Analítica and Calidad did not.
   - **Root cause (Analítica):** `IdentityPresenter::present()` — the payload `/auth/verify-code` returns and `useAuth()`'s `identity` object carries — never included an `analytics.view` key in its `abilities` map, even though `RoleSeeder` and the `seed_analytics_view_permission` migration correctly grant `analytics.view` to `super_admin`/`hr_agent`/`auditor`. `canViewAnalytics()`/`canViewCoverage()` on the frontend read `identity.abilities['analytics.view']`, which was always `undefined` → falsy, for every role. Every route-level test in `Sprint8AnalyticsAccessTest` still passed, because the `ability:analytics.view` middleware checks the Spatie grant directly and never goes through this presenter — the bug was invisible to every test that existed. Cobertura's nav entry happened to survive for `super_admin` only, via the pre-existing `analytics.view OR knowledge.edit` OR-fallback (super_admin holds `knowledge.edit` too) — `hr_agent`/`auditor`, who hold `analytics.view` but not `knowledge.edit`, would have had Cobertura's nav entry broken too, though nobody had logged in as them yet to notice.
   - **Root cause (Calidad):** Calidad was never a top-level nav entry at all — it was built as a tab nested inside the Review page (Step 7's `ReviewQueuePage`), even though `data-model.md`'s own access-control table already (incorrectly) described it as a peer nav entry alongside Analítica/Cobertura, and `roadmap.md`/`deploy.md` both describe "three new admin screens."
   - **Fix:** added the missing `'analytics.view' => $account->can('analytics.view')` key to `IdentityPresenter::present()` (and the same key to `AdminController::present()`, kept in lockstep per that file's own existing convention). Promoted `QualitySampleQueue` out of `ReviewQueuePage` into its own `AdminShell` view (`'quality'`), with a new `canViewQuality()` helper. Per ADR-0030 §5's actual decision (quality-sample review invents no new ability; reads are open to any admin, proven by `Sprint8AnalyticsAccessTest::test_quality_sample_reads_are_open_to_any_admin`) `canViewQuality()` is unconditional for any logged-in admin — mirroring the pre-existing Guardrails nav precedent (nav open to all; the one write action stays gated on `escalation.work` server-side). `data-model.md` and `deploy.md` corrected to stop claiming `analytics.view` gates Calidad's nav.
   - **New regression test:** `Sprint8AnalyticsAccessTest::test_identity_payload_carries_analytics_view_correctly` calls `IdentityPresenter::present()` directly and asserts the `analytics.view` boolean per role — the only way to catch a presenter-payload bug, since a route-hit test structurally cannot (it never goes through the presenter). Also extended `test_quality_sample_reads_are_open_to_any_admin` to cover `super_admin`/`hr_agent` (previously only `auditor`/`knowledge_editor` were checked).
2. **Cobertura's "Convenios con brecha total" list concatenated reason codes with no separator** (e.g. `EXPIRED_NO_SUCCESSORSALARY_PDF_NOT_IMPORTED`) — `CoveragePage.tsx` rendered each code as an adjacent bare `<code>` with no separating whitespace or visual boundary (React drops the literal newline/indentation between `.map()`'d JSX siblings). Fixed: each reason code is now its own `.badge-review` pill (reusing the existing warning-toned badge token, not a new color pair) inside a `.reason-badges` flex-gap wrapper — visually distinct chips, not a run-on string.
3. **Re-verified for real**: `tsc -b` clean, `vite build` clean (no new warnings), full backend suite green (576 tests). **Fourth redeploy this session** — `hr-backend 7c3ee2b`, `hr-frontend 6f276ca`, `hr-ai` unchanged, `hr-docs` at commit time — health green.

## 15. Eyes-on round 2 — leaf-click wiring, reason-code honesty, chart labels (2026-09-10)

Continuing the browser walkthrough past §14 surfaced four more real, small findings — three code bugs and one documentation gap:

1. **A coverage leaf (e.g. Navarra › Hostelería › Prosa ✗) opened nothing on click.** `CoveragePage.tsx` passed a no-op `onOpenDocument={() => {}}` to `<Hierarchy lens="coverage" .../>`, but even a real handler would not have helped: `HierarchyController::coverageCellLeaves()` never put a `doc_uuid`/`fact_uuid` on any coverage-lens leaf, gap or covered, because `CorpusCoverageService`'s cell methods (`proseCell()`/`salaryCell()`/`factsCell()`/`rulingsCell()`) only ever returned `covered`/`reason_code`/`detail` — never the underlying document/fact identifier.
   - **Fix (backend):** every cell method now returns the identifier of whichever document/fact is the actual reason the cell is covered or uncovered — the covering doc, the zero-chunk blocking doc, the historical doc, the unconverted salary PDF, the proposed-but-unverified fact. `coverageCellLeaves()` surfaces `doc_uuid`/`fact_uuid` on the leaf, or a `fix_link` (`AdminLinks::documents($convenioId)`, a new optional parameter) when genuinely nothing exists for that cell yet.
   - **Fix (frontend):** `CoveragePage.tsx` now mirrors `KnowledgeMapPage`'s exact leaf-opens-card wiring (real `onOpenDocument`/`onOpenFact` state, `DocumentDetailPanel`/`ReferenceFactPanel` rendered beside the map). `Hierarchy.tsx`'s `openLeaf()` falls back to `node.fix_link` (sets `window.location.hash`) when a leaf has neither a `doc_uuid` nor a `fact_uuid`. `DocumentsPage` gained an `initialConvenioId` prop (the same deep-link pattern `GroupsQueue`/`ReviewQueuePage` already use) plus a clearable convenio filter chip, so "no document at all" now lands the admin on Documents pre-filtered to the right convenio instead of nowhere.
   - **New test:** `CoverageLeafResolutionTest` — hits the real `/admin/hierarchy/children?lens=coverage` endpoint against fixtures for every prose/salary/facts gap cause (scan-no-text, under-review, expired-no-successor, PDF-not-imported, fact-needs-review, and each cell's true "nothing at all" case) and asserts every leaf resolves to a `doc_uuid`, `fact_uuid`, or `fix_link` — never none of the three.
   - **A real bug found by writing this test, not by staging data:** `KnowledgeMap::proseTypeIds()` memoized `document_type` ids in a PHP `static` local variable, commented "cached per request" but actually cached for the life of the PHP worker process — harmless in production (`document_types` never changes at runtime) but silently broke `CoverageLeafResolutionTest`'s later methods, because Postgres does not roll back sequence advances on `RefreshDatabase`'s per-test transaction rollback, so a later test's freshly-reseeded `document_types` got different ids than the first test's now-stale cached ones. Removed the memoization — the query is a cheap 3-row indexed lookup, called once per convenio per `grid()` call, not once per request.
2. **"Unclassified gap" badges on Salario/Resoluciones cells.** Both were falling through `CorpusCoverageService`'s own documented, intentional `reason_code = null` design (this class's docblock: "where no code fits, leave it null and state it in `detail`") into `coverageCellLeaves()`'s fallback of a fake `coverage_gap_unclassified` badge — which is exactly the "needs manual investigation" framing these two well-understood cases do not deserve.
   - **Salary, genuinely no source at all** (no table, no PDF): a real, deliberately-added reason code, `REASON_NO_SALARY_SOURCE`, replaces the null fallback — the tenth reason code, and the first added outside the original hand-run ledger's 8-code vocabulary, because this is a common, unambiguous case the ledger's vocabulary simply never named.
   - **Rulings, uncovered:** re-classified as *not a gap at all* — `rulingsCell()`'s doc-comment now says explicitly why (rulings are rare and opt-in, unlike prose/salary/facts, which every convenio is expected to eventually have; "no ruling" is a normal, expected state, not a coverage hole). `coverageCellLeaves()` special-cases it: no `gap_kind`, ever; the leaf's `meta` reads a neutral `—` instead of `✗` when uncovered.
   - `corpus-coverage.md` re-exported from real staging data after the redeploy below, since `toMarkdown()`'s salary column now shows `` `NO_SALARY_SOURCE` `` wherever it used to show a bare `✗`.
3. **Analítica's authority chart showed two bars both visually labelled "national…".** `.chart-bar-label`'s fixed 60px single-line ellipsis truncation hid that two `authority_used_key` values were genuinely distinct (e.g. `national_law` vs `national_law+official_convenio`, `DeflectionAnalytics::authorityKey()`'s `+`-joined sorted-array key) — indistinguishable even with the pre-existing `title` tooltip, since nothing on-screen hinted there was more to hover for. Fixed by wrapping the label over up to 3 lines in a wider column instead of truncating; the tooltip stays as a fallback for anything longer still.
4. **Documentation gap: staging test-account traffic contaminates cluster/analytics output.** Real test-account questions (e.g. `'pregunta de prueba fence 7d'`, `'capital de Mongolia'`) are indistinguishable from real employee traffic once they land in `message_traces` — expected behavior for a staging environment being exercised by hand, not a bug, but a real risk if go-live skips wiping staging's test-account history first. Added to `deploy.md` §4 alongside the existing `DEV-FIXTURE-0001` scrub item, with the concrete example queries named so the caveat can be `grep`'d for and confirmed gone post-wipe.

**Re-verified for real**: full backend suite green (586 tests, up from 576 — the ten `CoverageLeafResolutionTest` methods plus the fixed `proseTypeIds()` regression risk), `tsc -b` clean, `vite build` clean. **Fifth redeploy this session** — `hr-backend a1ab862358cc45a9a2f7d2f2e1036701842270b2`, `hr-frontend c711c9e0fc03fe292461aabfa879e474a94f890d`, `hr-ai` unchanged (`d1b7d3f4feea598106e93266e83a9fd4a1108602`), `hr-docs` at this commit — health green; `corpus-coverage.md` re-exported from the live post-deploy grid.

## 16. Employee chat produced no answer on staging — a pre-existing browser crash, not a Sprint 8 bug (2026-09-10)

`test-navarra@example.com` logged in and sent `'¿cuántos días de vacaciones tengo?'` from the real `/app` chat screen; no answer appeared. Diagnosed read-only, layer by layer, before touching anything:

1. **Did the request reach hr-backend?** No. Caddy's own access log (`hr-staging-caddy-1`, JSON access log, `handled request` per proxied call) shows **zero** `POST /api/chat/message` entries — ever, across this container's whole lifetime — despite the same log showing the employee's login (`POST /api/auth/verify-code`) and repeated `GET /api/chat/session` polling succeeding. The request never left the browser.
2. **Any new `chat_sessions`/`chat_messages`/`message_traces` row?** No — zero `chat_sessions` rows for this employee at all (confirmed directly against the RDS instance), consistent with #1: a session row is only ever created by a successful `POST /chat/message`, which never happened.
3. **hr-ai/Anthropic layer**, checked anyway per the explicit ask, and to rule out a second independent fault: `hr-ai:8001/health` → `200`, fast; `/route` reachable (`401` without the internal token — the same documented, expected shape as `/embed-batch`'s own 401, not a failure); `answer_model_settings` has a configured Claude key (`key_last_four: oAAA`, `configured_at: 2026-09-07`). All healthy.
4. **Root cause, found in `ChatScreen.tsx`:** `submit()` called `crypto.randomUUID()` to mint a local React list-key, **before its own `try`/`catch`**. `Crypto.randomUUID()` requires a secure context (HTTPS or `localhost`) in every modern browser — and staging's `Caddyfile` is deliberately plain `:80` (documented there as "the first-bring-up variant," no domain/TLS yet). On plain HTTP, `crypto.randomUUID` is `undefined`; calling it throws a `TypeError` that nothing catches, so `submit()` aborts *before* `sendChatMessage()` — and so the network request — ever runs. The textarea had already been cleared (`setInput('')` runs first), so the only visible symptom is exactly what was reported: the question disappears, and nothing else happens. Three more call sites in the same file (`pickCategory()`, and both assistant-item ids) had the identical problem, just unreached because the first throw always fires first.
   - **Not a Sprint 8 regression** — `git log -L` traces this line back to Sprint 2b-1 (`54a63e4`, the chat UI's original commit). It was invisible until now because nothing had exercised the employee `/app` chat screen in a real browser against this staging deployment before this eyes-on click — every earlier "real traffic" this session (Estatuto evidence, `questions:cluster` fixtures) went through the API directly, never through this screen.
   - **Not `message_feedback`/`ThumbsFeedback`, not the 7g neutral-message change** — those only render once an answer already exists in `items`; the crash happens on send, before any answer exists, so they were never reached.
5. **Fix:** replaced all four `crypto.randomUUID()` call sites with a `localId()` helper that feature-detects `crypto.randomUUID` and falls back to a `Date.now()`+`Math.random()` string when unavailable. These ids are purely local, ephemeral React keys, never sent to the server, so they never needed cryptographic randomness — the fallback is a correct fix, not a workaround, and behaves identically once a real HTTPS domain lands.
6. **Verified end-to-end for real**, read-only diagnosis first as asked, then the fix: after redeploying, called `POST /api/chat/message` through the real Caddy proxy (not directly to the container) with `test-navarra@example.com`'s own bearer token and the exact reported question — got back a real, grounded, cited `200` answer (`"Tienes derecho a 37 días laborables de vacaciones…"`, `outcome: answer`, `escalated: false`, 3 grounded claims, `synthesis.model: claude-sonnet-4-5`) — proving the entire backend→hr-ai→Anthropic path was never the problem. `tsc -b`/`vite build`/`eslint` all clean (no frontend test framework exists in this repo to add a regression test to — verification is the same tsc/build/eyes-on discipline every other frontend-only fix this session has used).

**Sixth redeploy this session** — `hr-frontend b1f715a2618f94c3a06b7cfa7c6aecf0cc2e7a84`, `hr-backend`/`hr-ai` unchanged from §15, `hr-docs` at this commit — health green.

## 17. Calidad eyes-on — reviewer audit gap, mislabeled question column, missing §6.5 summary (2026-09-11)

Three more real findings on the Calidad screen, all confirmed against real staging data (the 8 real `agent@hr-staging.internal` reviews from §11/§13) before any fix:

1. **REVISOR read '—' on all 8 reviewed rows — an ADR-0018 audit gap.** `reviewed_by` **was** written correctly on every verdict save (`QualitySamplingService::recordVerdict()` — confirmed unchanged, always has set `$sample->reviewed_by = $admin->id`). The bug was purely in what the API *returned*: `QualitySampleController` eager-loaded the `reviewedBy` relation and let Eloquent's default `toArray()` serialize it — but Eloquent keys a loaded relation under its **snake_case relation name**, `reviewed_by`, which collides with, and silently overwrites, the raw `reviewed_by` FK column already in the same array (`relationsToArray()` is merged in *after* `attributesToArray()`). The frontend, meanwhile, was reading a **third** key, `reviewed_by_admin`, that the backend never sent under any circumstance — so the column read '—' regardless of whether a review had happened, exactly the audit gap the user named (a verdict with no visible reviewer, ADR-0018).
   - **Fix:** `QualitySampleController::presentSample()` — one shared, hand-built response shape for `index()`/`show()`/`review()` (replacing ad-hoc `Eloquent::toArray()` for this endpoint family) — now sends two deliberately distinct, non-colliding keys: `reviewed_by` (the raw FK, unchanged) and `reviewer` (`{id, full_name}`, the loaded admin). Frontend now reads `reviewer.full_name`.
   - **New tests:** `Sprint8AnalyticsAccessTest::test_review_write_and_subsequent_reads_carry_the_acting_admin_as_reviewer` — posts a verdict as `hr_agent`, asserts the write response's `sample.reviewed_by` **and** `sample.reviewer.id` both equal the acting admin's id, then asserts the *same* is true on a subsequent list read and a subsequent detail read (not just the write echo).
2. **PREGUNTA showed the assistant's answer, not the employee's question.** `quality_samples.message_id` always points at the **assistant** turn (§6.1 — the drawn population is answered turns), so `message.content` — the only text field the table ever rendered — was always the answer. `QualitySamplingService` already had the exact "find the nearest preceding `role=user` message in the session" query, but only inside the private `openFixCard()` helper (used solely when a verdict opens a correction card); nothing exposed it to the read side.
   - **Fix:** extracted that lookup into a new public `QualitySamplingService::pairedUserMessage()` (used by both `openFixCard()` and the controller now — one lookup, not two). `presentSample()` adds a real `question` field. `QualitySampleQueue.tsx`'s PREGUNTA column now shows `question` as the primary text; the answer (`message.content`) is a `<details>/<summary>Ver respuesta</summary>` disclosure inside the same cell — expandable, not primary, and `stopPropagation()`'d so toggling it doesn't also open the row's detail drawer.
   - **New tests:** `Sprint8QualitySampleStratificationTest::test_paired_user_message_resolves_the_employee_question_not_the_answer` (service-level, asserts the resolved question ≠ the assistant's answer) and `Sprint8AnalyticsAccessTest::test_question_field_is_the_employee_message_not_the_answer` (HTTP-level, asserts `sample.question` on both `show()` and `index()` equals the fixture's `'pregunta de prueba'`, not `'respuesta de prueba'`).
3. **No monthly summary/trend rendered anywhere on Calidad**, despite plan.md §10 explicitly naming "quality-verdict trend §6.5" as one of the shared `BarChart`/`.chart-bars` primitive's intended call sites, and §6.5 itself specifying a live group-by (already built — `QualitySamplingService::monthlyTrend()`, the `GET /admin/quality-samples/trend` route) with a frontend client (`getQualityTrend()`) that nothing had ever called.
   - **Fix:** `QualityMonthlySummary`, rendered above the table in `QualitySampleQueue.tsx`. Aggregates the trend endpoint's `(month, verdict, stratum_path, stratum_territory_id, count)` rows down to `(month, verdict)` totals, renders one textual line per month (correcta / parcialmente / incorrecta counts + accuracy %), and one `BarChart` (the shared `.chart-bars` primitive, same component `AnalyticsPage`'s authority split uses) for the latest month's three verdict counts. Refetches after every verdict save, not just on page load.
   - **New test:** `Sprint8QualitySampleStratificationTest::test_monthly_trend_sums_to_the_real_verdict_counts` — records a known, mixed set of 6 verdicts (4 correct / 1 partially / 1 wrong) via `recordVerdict()`, then asserts `monthlyTrend()`'s rows, aggregated by `(month, verdict)` exactly as the frontend does, sum to those exact counts. This is the available substitute for a frontend assertion (no frontend test framework exists in this repo, per §16's note) — it proves the data contract the on-screen summary is built from is correct.
   - **Confirmed against real staging data post-deploy:** the 2026-09 line reads `7 correcta · 1 parcialmente · 0 incorrecta · 87.5%` — matching the 8 real `agent@hr-staging.internal` reviews from earlier this session exactly.

**Re-verified for real**: full backend suite green (**590 tests**, up from 586 — 2 new methods in `Sprint8QualitySampleStratificationTest`, 2 new methods in `Sprint8AnalyticsAccessTest`), `tsc -b` clean, `eslint` clean, `vite build` clean. **Seventh redeploy this session** — `hr-backend fb6a46a5d2ce13b30574140aec8627b57cbdd069`, `hr-frontend e7fa65d906745792be38fa42deeb46e6d69e3488`, `hr-ai` unchanged (`d1b7d3f4feea598106e93266e83a9fd4a1108602`), `hr-docs` at this commit — health green.
