#!/usr/bin/env python3
"""Sprint 10c vacaciones segmentation eval scorer (plan §D.10, CP-B) — stdlib
only. Copied unmodified in logic from eval/permisos/score_eval.py (CP-A) —
the scoring shape (keyword-matched positive/negative gold entries, digit-set
value matching, 0-tolerance scope gate) is topic-agnostic; only the gold set
and quotes differ per topic.

THE EVAL IS THE DELIVERABLE (same posture as 7b-2's and permisos' scorers).

Structural note (same as permisos): this driver calls hr-ai ONCE PER
CONVENIO (single-candidate binding), so scope reduces to "did every proposed
fact carry the call's own convenio_id" (checked directly, 0-tolerance).  For
vacaciones, the real risk this topic's real corpus surfaces is NOT
cross-convenio mis-scope, it's a TOPIC/ARTICLE-BOUNDARY trap of a different
shape than permisos' lactancia leak: real convenios interleave a
JORNADA-hours schedule (c.20) or a bare jornada-hours figure (c.10) in the
same paragraph block or across a page-break running-header repeat as the
vacaciones day-count article, with no topic-anchor overlap in the WORD sense
(no shared word like "permiso") — the leak risk here is purely
document-structural/OCR-pagination, not lexical. Matching here is by MOTIVO
KEYWORD (a gold entry's `keywords`, matched against a proposed fact's
