#!/usr/bin/env python3
"""Sprint 10c festivos segmentation eval scorer (topic 4, next checkpoint
after CP-3) — stdlib only. Copied unmodified in logic from
eval/jornada/score_eval.py, itself unmodified (bar one norm() fix) from
eval/vacaciones/score_eval.py (CP-B) and eval/permisos/score_eval.py (CP-A)
— the scoring shape (keyword-matched positive/negative gold entries,
digit-set value matching, 0-tolerance scope gate) is topic-agnostic; only
the gold set and quotes differ per topic.

THE EVAL IS THE DELIVERABLE (same posture as 7b-2's and every prior
topic's scorer).

Structural note (same as permisos/vacaciones/jornada): this driver calls
hr-ai ONCE PER CONVENIO (single-candidate binding), so scope reduces to
"did every proposed fact carry the call's own convenio_id" (checked
directly, 0-tolerance). For festivos, this eval's own real shape — measured
BEFORE writing the gold set, not assumed — is that RESTRAINT is the primary
dimension, not recall: TopicLexicon's 'festivos' anchor is corpus-wide
dominated by retribución content (a per-hour/per-day premium for WORKING on
a festivo), and only 2 of the 6 eval convenios (c.2, c.10) have any genuine
calendar-of-festivos-days content at all. The other 4 are correct-zero-yield
cases, with c.18 as the hardest trap (real specific dates present, but only
to bound plus-eligibility, never a día-de-cierre grant) and c.10 carrying a
same-article adjacency trap (23.3's retribución sub-clause sits immediately
before 23.4's genuine libranza fact). Matching here is by MOTIVO KEYWORD (a
gold entry's `keywords`, matched against a proposed fact's
`value`+`source_excerpt`), not group/scope.
