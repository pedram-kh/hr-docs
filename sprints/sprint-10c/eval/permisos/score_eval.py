#!/usr/bin/env python3
"""Sprint 10c permisos segmentation eval scorer (plan §D.10) — stdlib only.

THE EVAL IS THE DELIVERABLE (same posture as 7b-2's score_eval.py, adapted for
the single-convenio, topic-parameterized driver introduced this sprint).

Structural difference from 7b-2's scorer: 7b-2 scored CROSS-PROVINCE scope
matching (territory/sector/group, because one call segmented 27 convenios at
once). This driver calls hr-ai ONCE PER CONVENIO (single-candidate binding),
so scope reduces to "did every proposed fact carry the call's own
convenio_id" (checked directly, 0-tolerance per plan §D.10) — the real risk
this topic's real corpus surfaces is not cross-convenio mis-scope, it's
TOPIC mis-identification: several real convenios embed maternidad/lactancia/
paternidad clauses (statutory suspension-of-contract machinery) inside or
beside the very same "Permisos"/"Licencias" article, anchored on the same
word ("permiso"). Matching here is by MOTIVO KEYWORD (a gold entry's
`keywords`, matched against a proposed fact's `value`+`source_excerpt`), not
group/scope.
