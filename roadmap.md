...
- The four **roles** wired through the UI (`super_admin`, `hr_agent`, `knowledge_editor` no-chat, `auditor` read-only) via `spatie/laravel-permission`, including **role-scoped conversation access** (pillar §4.2 — an `hr_agent` sees escalated chats, not everyone's history). **Reaffirmed from Sprint 4:** the **full-history browser** (beyond Sprint 4's card-scoped view) and role-scoped-access **enforcement** land here, plus the **history-search** UI moved from 2b.
