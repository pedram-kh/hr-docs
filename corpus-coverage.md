...
**Query date/time:** 2026-09-08 02:03 UTC, live against `hr-staging`'s DB + S3 (source of truth — not `deploy.md`).
**Commit SHAs staging is running at query time:** `hr-backend` `0a54a6a`, `hr-ai` `530276c`, `hr-frontend` `0aabb92`, `hr-docs` `ef12475` (all on branch `sprint-7e`, one commit ahead of `main`; the OCR fallback + Sprint-7e frontend fixes are live).
**Corpus size at query time:** 101 documents, 26 registry convenios, 3 employees seeded, 3,534 chunks in `document_chunks`, 0 rows in `reference_facts`.
...
| 12 | 20104415012022_Gestores informacion Gipuzkoa_Tablas 2025.pdf | salary_tables | 15 INFORMACIÓN Y DOCUMENTACIÓN | Gipuzkoa | active | auto_proposed | — | 3(3) | — | 0 | no | `SALARY_PDF_NOT_IMPORTED` | re-supply as `.xlsx`, or extend `salary:import` to parse PDF tables |
...
| 27 | 20104415012022_Gestores informacion Gipuzkoa_Tablas 2026.pdf | salary_tables | 15 INFORMACIÓN Y DOCUMENTACIÓN | Gipuzkoa | active | auto_proposed | — | 3(3) | — | 0 | no | `SALARY_PDF_NOT_IMPORTED` | same convenio's prose is fine; salary still blocked |
...
| 57 | 01100635012017 OCIO EDUCATIVO ALAVA 2023-2026_Tablas 2026.pdf | salary_tables | 3 OCIO EDUCATIVO Y ANIMACIÓN SOCIOCUL | Álava | active | auto_proposed | — | 3(3) | — | 0 | no | `SALARY_PDF_NOT_IMPORTED` | re-supply as `.xlsx` |
...
| 59 | 01100635012017 OCIO EDUCATIVO ALAVA 2023-2026_Tablas 23-25.pdf | salary_tables | 3 OCIO EDUCATIVO Y ANIMACIÓN SOCIOCUL | Álava | active | auto_proposed | — | 3(3) | — | 0 | no | `SALARY_PDF_NOT_IMPORTED` | re-supply as `.xlsx` (also superseded period) |
...
| 94 | 33000325011978_DEPORTES ASTURIAS_TABLAS 2013.pdf | salary_tables | 5 DEPORTE ASTURIAS | Asturias | active | auto_proposed | — | 2(2) | — | 0 | no | `SALARY_PDF_NOT_IMPORTED` | re-supply as `.xlsx` |
...
| `SALARY_PDF_NOT_IMPORTED` (PDF-format salary table) | 15 (12,19,24,27,32,35\*,43,46,57,59,86,88\*,94,97\*,12) | ~40 | 12,15,22,3(×2),18,8,5 (several already salary-OK via a separate `.xlsx`) | 0 | `salary:import` is `.xlsx`-only by design; either re-supply as `.xlsx` or extend the importer to parse tabular PDF pages | sprint-8 decision |
...
| `OK` (answerable today) | **28 prose + 8 salary = 36** | — | 2,3,6,8,10,12,13,15,17(salary only? no— see below),18,19,22,25 | 3 | — | — |
...
- 15 PDF-format salary tables — action: re-supply as `.xlsx`, the one format the importer reads.
...
- 15 PDF-format salary tables (`salary:import` is `.xlsx`-only — a scope decision, not a bug).
...
| 3 Ocio Educ. Y Anim. Sociocul. (Álava) | ✓ (doc 56) | ✗ (PDF only) | ✗ | doc 64 "Cambios" not embeddable (no flow) |
| 4 Ocio Educ. Y Anim. (Andalucía) | ✗ `EXPIRED_NO_SUCCESSOR` | ✗ (untagged) | ✗ | doc 89 expired; docs 10/11/90 unbound |
| 5 Deporte (Asturias) | ✗ `EXPIRED_NO_SUCCESSOR` (2012) | ✗ (PDF only) | ✗ | **full gap** |
...
| 15 Información y Documentación (Gipuzkoa) | ✓ (doc 26) | ✗ (PDF only) | ✗ | — |
...
Re-run this ledger after every ingest, OCR run, verify session, salary import, or reference-fact batch — the query date/time at the top is the freshness signal; if it's more than a few days old relative to the last ingest, treat the numbers as stale. All figures above came from direct queries against the live staging DB (documents, document_pages, document_chunks, salary_tables, salary_table_rows, reference_facts, convenios, employees) via `php artisan tinker`, not from any markdown doc.
