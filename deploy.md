...
- [ ] **Scanned, no-extractable-text convenios → currently unanswerable; now fixable (Sprint 7e).** Some registry-`active` convenios are **image-only PDFs** with no extractable text, so they produce **no chunks** and the chat cannot answer from them: **`PACTO CULTURA NAVARRA` (id 31)** and **`CONVENIO DEPORTE NAVARRA 2025–2028` (id 50)** (both also `under_review`; ids corrected from the original 91/89 — a re-ingest since this item was first written renumbered every document, `sprint-07e/plan.md` §1.7). **COEAS Estatal is NOT one of these** — corrected above (§4b): it already has a full text layer; its `eligible: 0` finding is a separate embedding/scope-state gap, unrelated to OCR. *Pre-go-live action:* `documents:ocr-backfill` (Sprint 7e) OCRs docs 31/50/14 + six further historical documents (the corpus's actual 9-document text-less inventory), each left `under_review` for human verification, then `chunks:embed` + the §4.4 acceptance check (chunks > 0, in retrieval, a scoped cited answer) per target — see `sprint-07e/review.md` §3 for the run report. An **explicit scope exclusion** remains the fallback for any target that doesn't clear verification.
