...
- **Hardcoded baseline** (admins cannot weaken): no legal/medical advice, no answers about other employees' data, immediate human escalation for sensitive topics (harassment, mental health, disciplinary/termination), and **no salary answers in chat** (salary is structured data in `salary_tables`, never a prose chunk — escalate with reason `salary_not_in_chat`; salary-in-chat is 2b-2).
...
>
> **Salary-topic guard (Correction-02).** The baseline also catches salary/wage/retribution questions (`salario`, `sueldo`, `nómina`, `retribución`, `tablas salariales`, `cuánto gano/cobra…`) and escalates them with the distinct reason **`salary_not_in_chat`** before any `hr-ai` call — never synthesising. This enforces ADR-0006: salary lives in the structured `salary_tables` (a figure must be *exact*) and is never embedded as a vector chunk. It closes a live bug found in adversarial testing: a salary question retrieved a **wage-table page that 2a had chunked as prose** and surfaced a **year-misattributed** figure (2024 reported as the 2025 row), which the figure-grounding guard could not catch because it validates digit presence, not row/column/year alignment. The bare verb "paga" is deliberately *not* matched (so "¿la empresa me paga el gimnasio?" is not a salary question). The grounded salary answer (from `salary_tables` via SQL) is **Sprint 2b-2**, which supersedes this guard. *Known residual (accepted):* the 2a wage-table prose chunks remain in the index; this guard stops salary questions from reaching them, but a non-salary question could still retrieve a table chunk — the deeper 2a fix (excluding wage-table pages from prose chunking) is parked for later.
