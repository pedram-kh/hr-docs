...
| employee_id | bigint FK → employees | |
| reason | enum | `low_confidence` \| `sensitive_topic` \| `off_domain` \| `explicit_request` \| `conflict` \| `salary_coverage_gap` (2b-2) \| `reference_fact_coverage_gap` (7c, ADR-0023 — no verified, in-scope, in-validity fact answers; the only 7c schema migration) \| ~~`salary_not_in_chat`~~ (retired in 2b-2 — kept in the enum for historical rows, no longer emitted) |
