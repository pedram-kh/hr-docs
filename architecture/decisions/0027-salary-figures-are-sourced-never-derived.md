...
- **`base_salary_monthly` is NULL for more rows than before, and correct where it is not.** Across the corpus this was 4 tables gaining a correct sourced monthly they never had, 3 tables having a wrong one replaced, and 2 tables (Cantabria) unchanged because their source's own `Salario base` happens to equal annual/14 — which is the coincidence that let the assumption survive review in Sprint 2a.
