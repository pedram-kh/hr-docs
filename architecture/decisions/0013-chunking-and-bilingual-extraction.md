...
## Decision
- **Article-aware chunking with a size cap.** Split on article boundaries; sub-split oversized articles; pack tiny ones. Record `page_from`/`page_to` and `chunk_index`.
